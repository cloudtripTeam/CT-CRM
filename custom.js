...
$(document).ready(function(){
    ...
    $('.quantity-selector').quantitySelectorChild();
    $('.quantity-selector').quantitySelectorInfant();
   autocomplete();
    ...
    $('.typeahead').typeahead({
        ...
        source: function (q, cb) {
            return $.ajax({
                dataType: 'jsonp',
                type: 'get',               
                url: 'https://www.flightspro.co.uk/autocompleteJson.ashx?callback=?&q=' + q,
                chache: false,
                success: function (data) {
                    var res = [];                   
                    $.each(data, function (index, val) {
                       
                        if (val.AirportCode !== "%s") {
                            res.push({
                                id: index,
                                name: val.CityName + "-" + val.AirportName + ":" + " [" + val.AirportCode + "]"
                            })
                        }
                    })
                    cb(res);
