...
public partial class Admin_MarkupUk : CompressedPage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Session["UserDetails"] != null)
        {
            if (!IsPostBack)
            {
                ...
                if (!objUserDetail.isAuth("Markup"))
                {
                ...
                if (objUserDetail.userRole.ToLower() == "fares")
                {
                    ONOFF.Style.Add("visible", "false");
                }
                ...
                checkboxisActive.Checked = CheckDiscountMarkup_UK();
                checkboxisActive2.Checked = CheckDiscountMarkup_CA();
    ...
    public static string DeleteMarkup(string ID)
    {
        ...
        return objGetSetDatabase.DeleteFlightMarkupUK(ID, "Delete").ToString().ToLower();
    }
    ...
    public static string UpdateMarkupExcel(string ID, string UpdateField, string Value, string UpdatedBy)
    {
        try
        {
            ...
            return objGetSetDatabase.UpdateMarkupExcelUK(ID, UpdateField, Value.Trim().ToUpper(), UpdatedBy);
        }
        catch { return "false"; }
    }

    [WebMethod(EnableSession = true)]
    public static string UpdateDiscountMarkup_UK(string UseStatus)
    {
        try
        {
            GetSetDatabase objGetSetDatabase = new GetSetDatabase();
            return objGetSetDatabase.UpdateDiscountMarkup_UK(Convert.ToBoolean(UseStatus));

        }
        catch { return "false"; }
    }
    [WebMethod(EnableSession = true)]
    public static string UpdateDiscountMarkup_CA(string UseStatus)
    {
        try
        {
            GetSetDatabase objGetSetDatabase = new GetSetDatabase();
            return objGetSetDatabase.UpdateDiscountMarkup_CA(Convert.ToBoolean(UseStatus));
    ...
    public bool CheckDiscountMarkup_UK()
    {
        ...
        {
            GetSetDatabase objGetSetDatabase = new GetSetDatabase();
            return objGetSetDatabase.CheckDiscountMarkup_UK();
    ...
    public bool CheckDiscountMarkup_CA()
    {
        ...
        {
            GetSetDatabase objGetSetDatabase = new GetSetDatabase();
            return objGetSetDatabase.CheckDiscountMarkup_CA();
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        ...
        foreach (string cls in markupcalss)
        {
            ...
            {
                bool isgroup = false;
                if (ddlGroupCode.SelectedValue == "1")
                {
                    isgroup = true;
                }


                retStatus = objGetSetDatabase.SET_FlightMarkupDetailUK("", txtMarkupFrom.Text.Trim().ToUpper(),
                ...
                Convert.ToInt32(ddlPax.SelectedValue), Convert.ToInt32(txtDTD.Text.Trim()), txtStops.Text, isgroup, txtDBD.Text);
            }
        }


        if (retStatus.ToLower() == "true" || retStatus.ToLower() == "1")
        {
    ...
    private void bindMarkUp()
    {
        GetSetDatabase objGetSetDatabase = new GetSetDatabase();
        UserDetail objUserDetail = HttpContext.Current.Session["UserDetails"] as UserDetail;
        DataTable dt = objGetSetDatabase.GET_FlightMarkupDetailUK("", txtMarkupFrom.Text.Trim(), txtMarkupTo.Text.Trim(), txtAirV.Text.Trim(), "",
       Convert.ToString(ddlCategory.SelectedValue), txtMarkupClass.Text.Trim(), "", ddlCompany.SelectedValue, ddlCampDetails.SelectedValue, ddlJourneyType.SelectedValue, "", "",
    ...
    protected void btnExport1_Click(object sender, EventArgs e)
    {
        ...
        DataTable dt = (DataTable)ViewState["Markups"];
        if (dt != null)
        {
            string attachment = "attachment; filename=" + "MarkupUK_" + DateTime.Now.ToString("ddMMMyyyy") + ".xlsx";
    ...
    protected void btnExport_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            using (SqlConnection exportConn = DataConnection.GetConnectionMarkup())
            {
                ...
                "isnull([Agnt_AirF_Markup_ModifiedBy],'') AS ModifiedBy, isnull([Agnt_AirF_Markup_LastModifiedDate],'') AS LastModifiedDate  ,isnull([DaysToDeparture],'') AS DaysToDeparture, isnull(Agnt_AirF_No_Of_Pax,1) as [PaxCount],isnull([Agnt_AirF_Stops],'') AS Stops,IsGroupCode, DaysBetweenDeparture FROM [FlightMarkupDetail_UK]";
    ...
    protected void btnUpload_Click(object sender, EventArgs e)
    {
        ...
        {
            lblMsg.Text = "Please choose an excel markup file to proceed";
            ...
            using (DbDataReader dr = OleDbCmd.ExecuteReader())
            {
                ...
                while (dr.Read())
                {
                    dt.Rows.Add(CreateMarkupDtaRow(dt.NewRow(), dr["SNo"].ToString(), dr["From"].ToString(),
                        ...
                        dr["PaxCount"].ToString(), dr["Stops"].ToString(), dr["IsGroupCode"].ToString(), dr["DaysBetweenDeparture"].ToString()));
                }
                ...
                using (SqlConnection SqlCon = DataConnection.GetConnectionMarkup())
                {
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        ...
                        cmd.CommandText = "AirFareMarkup_BulkCopy_UK";
    ...
    private System.Data.DataTable CreateMarkupDataTable()
    {
        ...
        dt.Columns.Add("Agnt_AirF_Stops", typeof(string));
        dt.Columns.Add("IsGroupCode", typeof(string));
        dt.Columns.Add("DaysBetweenDeparture", typeof(string));
        return dt;
    }
    ...
    private DataRow CreateMarkupDtaRow(DataRow dr, string SNO, string From, string To, string AirV, string Provider, string Category,
        ...
        string CampaignID, string JourneyType, string ModifiedBy, ref int ctr, string DaysToDeparture, string PaxCount, string Stops, string IsGroupCode,string DaysBetweenDeparture)
    {
        try
        {
            ...
            dr["Agnt_AirF_Stops"] = Stops;
            dr["IsGroupCode"] = IsGroupCode;
            dr["DaysBetweenDeparture"] = DaysBetweenDeparture;
        }
        catch (Exception ex)
        {
    ...
    protected void btnDeleteAll_Click(object sender, EventArgs e)
    {
        ...
        {
            GetSetDatabase objGetSetDatabase = new GetSetDatabase();
            if (objGetSetDatabase.DeleteAllFlightMarkupUK("Truncate"))
            {
