...
public partial class Admin_OwnFaresJazz_US : Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Session["UserDetails"] != null)
        {
            if (!IsPostBack)
            {
                ...
                if (objUserDetail.isAuth("OwnFaresJazz_US.aspx"))
                {
                ...
                checkboxisActive.Checked = CheckDiscountOwnFareJazz_US();
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        string str = objGetSetDatabase.SET_ChangeFare_JazzUS("", txtOrigin.Text.Trim().ToUpper(), txtDestination.Text.Trim().ToUpper(), txtTravelDateStart.Text.Trim().ToUpper(),
        ...
        ddlFlightType.Text, txtDTD.Text == "" ? 0 : Convert.ToInt32(txtDTD.Text.Trim()), txtMarkup.Text);

        if (str.ToLower() == "true")
        {
            lblMsg.Text = "Record is successfully inserted";
            rptrDetails.DataSource = objGetSetDatabase.GET_ChangeFare_JazzUS("", txtOrigin.Text.Trim().ToUpper(), txtDestination.Text.Trim().ToUpper(),
    ...
    protected void btnUpdate_Click(object sender, EventArgs e)
    {
        GetSetDatabase objGetSetDatabase = new GetSetDatabase();
        UserDetail objUserDetail = HttpContext.Current.Session["UserDetails"] as UserDetail;
        string str = objGetSetDatabase.SET_ChangeFare_JazzUS(hfSrNo.Value, txtOrigin.Text.Trim().ToUpper(), txtDestination.Text.Trim().ToUpper(), txtTravelDateStart.Text.Trim().ToUpper(),
        txtTravelDateEnd.Text.Trim().ToUpper(), txtAieline.Text.Trim().ToUpper(), ddlCabinClass.SelectedValue, txtAirlineClass.Text.Trim().ToUpper(), txtBaseFare.Text.Trim(),
    ...
    public static string DeleteFaresJazzUS(string ID)
    {
        GetSetDatabase objGetSetDatabase = new GetSetDatabase();
        UserDetail objUserDetail = HttpContext.Current.Session["UserDetails"] as UserDetail;
        return objGetSetDatabase.DeleteOwnFares_JazzUS(ID, "Delete").ToString().ToLower();
    }

    [WebMethod(EnableSession = true)]
    public static string UpdateFareJazz_US(string ID, string UpdateField, string Value)
    {
        GetSetDatabase objGetSetDatabase = new GetSetDatabase();
        UserDetail objUserDetail = HttpContext.Current.Session["UserDetails"] as UserDetail;
        return objGetSetDatabase.UpdateOwnFare_US_Excel_JazzUS(ID, UpdateField, Value, objUserDetail.userID);
    }
    ...
    public void WriteExcelWithNPOI(DataTable dt, String extension)
    {
        ...
        using (var exportData = new MemoryStream())
        {
            ...
            string fileName = "OwnFaresJazz_US_" + DateTime.Now.ToString("ddMMMyyyy");
    ...
    protected void btnUpload_Click(object sender, EventArgs e)
    {
        ...
        if (FileUpload1.HasFile)
        {
            ...
            if (!Directory.Exists(Server.MapPath("~/Admin/OwnFareJazzUS/")))
            {
                Directory.CreateDirectory(Server.MapPath("~/Admin/OwnFareJazzUS/"));
            }
            string FilePath = Server.MapPath("~/Admin/OwnFareJazzUS/" + FileName);
    ...
    public void SaveRows(string FilePath, string Extension, string isHDR)
    {
        ...
        using (OleDbConnection OleDbCon = new OleDbConnection(sExcelconnectionstring))
        {
            ...
            OleDbCon.Close();
            DataTable dt = CreateOwnFareDataTable_JazzUS();
            ...
            using (DbDataReader dr = OleDbCmd.ExecuteReader())
            {
                ...
                using (SqlConnection SqlCon = DataConnection.GetConnectionMarkupUS())
                {
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        ...
                        cmd.CommandText = "OwnFare_BulkCopyFor_JazzUS";
    ...
    private DataTable CreateOwnFareDataTable_JazzUS()
    {
    ...
    private void truncateRecords()
    {
        try
        {
            ...
            GetSetDatabase objGetSetDatabase = new GetSetDatabase();
            if (objGetSetDatabase.DeleteOwnFares_JazzUS("", "Truncate") == "true")
            {
    ...
    private void DeleteSeletedFare(string CompaignID)
    {
        try
        {
            using (SqlConnection exportConn = DataConnection.GetConnectionMarkupUS())
            {
                ...
                exportConn.Open();
                string filter = string.Empty;
                string cmdstr = "Delete FROM [ChangeFare_JazzUS] WHERE Change_Fare_Campaign = '" + CompaignID + "'";
    ...
    public void CopyFares(object sender, EventArgs e)
    {
        ...
        try
        {
            using (SqlConnection conection = DataConnection.GetConnectionMarkupUS())
            {
                ...
                ds = SqlHelper.ExecuteDataset(conection, CommandType.StoredProcedure, "Insert_Copy_ChangeFare_JazzUS", param);
    ...
    public void Bind_OwnFareUS_DataGrid()
    {
        GetSetDatabase objGetSetDatabase = new GetSetDatabase();
        UserDetail objUserDetail = HttpContext.Current.Session["UserDetails"] as UserDetail;
        rptrDetails.DataSource = objGetSetDatabase.GET_ChangeFare_JazzUS("", txtOrigin.Text.Trim().ToUpper(), txtDestination.Text.Trim().ToUpper(), txtTravelDateStart.Text,
    ...
    [WebMethod(EnableSession = true)]
    public static string UpdateDiscountOwnFareJazz_US(string UseStatus, string Origin, string Destination, string Airline, string AirlineClass, string TripType, string OfferType, string Pax, string FlightType, string Campaign)
    {
        try
        {
            GetSetDatabase objGetSetDatabase = new GetSetDatabase();
            ...
            return objGetSetDatabase.UpdateDiscountOwnFare_JazzUS(Convert.ToBoolean(UseStatus), Origin, Destination, Airline, AirlineClass, TripType, OfferType, Pax, FlightType, Campaign);
        }
        catch
        {
            return "false";
    ...
    public bool CheckDiscountOwnFareJazz_US()
    {
        ...
        {
            GetSetDatabase objGetSetDatabase = new GetSetDatabase();
            return objGetSetDatabase.CheckDiscountOwnFareJazz_US();
