...
public partial class Admin_MarkupJazzUS : CompressedPage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Session["UserDetails"] != null)
        {
            if (!IsPostBack)
            {
                ...
                if (!objUserDetail.isAuth("MarkupJazzUS"))
                {
                ...
                checkboxisActive.Checked = CheckDiscountMarkup_JazzUS();
    ...
    public static string DeleteMarkupJazzUs(string ID)
    {
        GetSetDatabase objGetSetDatabase = new GetSetDatabase();
        return objGetSetDatabase.DeleteFlightMarkupJazz_US(ID, "Delete").ToString().ToLower();
    }

    [WebMethod(EnableSession = true)]
    public static string UpdateMarkupExcel_JazzUS(string ID, string UpdateField, string Value, string UpdatedBy)
    {
        try
        {
            GetSetDatabase objGetSetDatabase = new GetSetDatabase();

            return objGetSetDatabase.UpdateMarkupExcelJazz_US(ID, UpdateField, Value.Trim().ToUpper(), UpdatedBy);
        }
        catch { return "false"; }
    }

    [WebMethod(EnableSession = true)]
    public static string UpdateDiscountMarkupJazz_US(string UseStatus)
    {
        try
        {
            GetSetDatabase objGetSetDatabase = new GetSetDatabase();
            return objGetSetDatabase.UpdateDiscountMarkupJazz_US(Convert.ToBoolean(UseStatus));

        }
        catch { return "false"; }
    }

    public bool CheckDiscountMarkup_JazzUS()
    {
        try
        {
            GetSetDatabase objGetSetDatabase = new GetSetDatabase();
            return objGetSetDatabase.CheckDiscountMarkupJazz_US();
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        ...
        foreach (string cls in markupcalss)
        {
            ...
            {
                retStatus = objGetSetDatabase.SET_FlightMarkupDetailJazz_US("",
                                             txtMarkupFrom.Text.Trim().ToUpper(),
                                             txtMarkupTo.Text.Trim().ToUpper(),
                                             txtAirV.Text.Trim().ToUpper(),
                                             "ANY",
                                             ...
                                             txtDTD.Text == "" ? 0 : Convert.ToInt32(txtDTD.Text.Trim()),
    ...
    private void bindMarkUp()
    {
        GetSetDatabase objGetSetDatabase = new GetSetDatabase();
        UserDetail objUserDetail = HttpContext.Current.Session["UserDetails"] as UserDetail;
        DataTable dt = objGetSetDatabase.GET_FlightMarkupDetailJazz_US("", txtMarkupFrom.Text.Trim(), txtMarkupTo.Text.Trim(), txtAirV.Text.Trim(), "",
    ...
    protected void btnExport1_Click(object sender, EventArgs e)
    {
        ...
        if (dt != null)
        {
            string attachment = "attachment; filename=" + "MarkupJazzUS_" + DateTime.Now.ToString("ddMMMyyyy") + ".xlsx";
    ...
    protected void btnExport_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            using (SqlConnection exportConn = DataConnection.GetConnectionMarkupUS())
            {
                ...
                "isnull([Agnt_AirF_Markup_ModifiedBy],'') AS ModifiedBy, isnull([Agnt_AirF_Markup_LastModifiedDate],'') AS LastModifiedDate  ,isnull([DaysToDeparture],'') AS DaysToDeparture, isnull(Agnt_AirF_No_Of_Pax,1) as [PaxCount],isnull([Agnt_AirF_Restricted_Class],'') AS RestrictedClass FROM [FlightMarkupDetail_Jazz]";
    ...
    public void WriteExcelWithNPOI(DataTable dt, String extension)
    {
        ...
        using (var exportData = new MemoryStream())
        {
            ...
            string fileName = "MarkUpJazzUS_" + DateTime.Now.ToString("ddMMMyyyy");
    ...
    protected void btnUpload_Click(object sender, EventArgs e)
    {
        ...
        if (FileUpload1.HasFile)
        {
            ...
            if (!Directory.Exists(Server.MapPath("~/Admin/MarkupJazzUS/")))
            {
                Directory.CreateDirectory(Server.MapPath("~/Admin/MarkupJazzUS/"));
            }
            string FilePath = Server.MapPath("~/Admin/MarkupJazzUS/" + FileName);
    ...
    public void SaveRows(string FilePath, string Extension, string isHDR)
    {
        ...
        using (OleDbConnection OleDbCon = new OleDbConnection(sExcelconnectionstring))
        {
            ...
            OleDbCon.Close();
            System.Data.DataTable dt = CreateMarkupDataTable_JazzUS();
            ...
            using (DbDataReader dr = OleDbCmd.ExecuteReader())
            {
                ...
                using (SqlConnection SqlCon = DataConnection.GetConnectionMarkupUS())
                {
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        ...
                        cmd.CommandText = "AirFareMarkup_BulkCopy_JazzUS";
    ...
    private DataTable CreateMarkupDataTable_JazzUS()
    {
    ...
    protected void btnDeleteAll_Click(object sender, EventArgs e)
    {
        ...
        {
            GetSetDatabase objGetSetDatabase = new GetSetDatabase();
            if (objGetSetDatabase.DeleteAllFlightMarkupJazz_US("Truncate"))
            {
