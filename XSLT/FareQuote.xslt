  ...
  <xsl:template match="Itinerary">
    ...
    <div class="panel panel-default">
      <div class="panel-heading">
        <big>
          Fares from (<xsl:value-of select="Sectors/Sector[isReturn = 'false'][1]/Departure/CityName"/>) to (<xsl:value-of select="Sectors/Sector[isReturn = 'false'][last()]/Arrival/CityName"/>),
        </big>
        <small>
          <xsl:value-of select="Sectors/Sector[isReturn = 'false'][1]/CabinClass/Des"/> Class on <xsl:variable name="month">
        ...
        <span style="float: right; padding-right:20px; " class="bluetext text-primary  text-right  ">
          <!--Flights Duration: 14:05 hrs.-->
          <xsl:for-each select="Sectors/Sector[position()=$InboundNo]">
            Flights Duration:<xsl:value-of select="concat(substring(ActualTime,1,2),'')"/>:<xsl:value-of select="concat(substring(ActualTime,4,2),'')"/>hrs
          </xsl:for-each>
        </span>
      </div>
